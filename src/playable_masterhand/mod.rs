...
pub unsafe fn check_status() -> bool {
    EXISTS_PUBLIC
        || boss_runtime::any_exists_public(&raw const boss_runtime::PLAYABLE_MASTERHAND_RUNTIME)
}
...
unsafe fn log_playable_masterhand_transition_snapshot(
    ...
    let flags = (selected_via_slot as u16)
        | ((cpu_entry as u16) << 1)
        | ((ready_go as u16) << 2)
...
unsafe fn apply_smoothed_playable_masterhand_input(
    ...
    update_smoothed_control_axis(
        &raw mut CONTROLLER_X,
        ControlModule::get_stick_x(module_accessor),
    );
    update_smoothed_control_axis(
        &raw mut CONTROLLER_Y,
    ...
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            let _runtime_guard = CommonRuntimeSyncGuard::new(
                boss_runtime::slot_ptr(
                    &raw mut boss_runtime::PLAYABLE_MASTERHAND_RUNTIME,
                    ENTRY_ID,
                ),
            ...
            let selected_via_slot =
                selection::is_selected_css_boss(module_accessor, *ITEM_KIND_PLAYABLE_MASTERHAND);
            log_playable_masterhand_transition_snapshot(
                module_accessor,
                fighter_manager,
                selected_via_slot,
            );
                ...
                if boss_helpers::is_boss_preview_stage(stage_id) {
                    ...
                    if ModelModule::scale(module_accessor) == 0.0001 {
                            ...
                            && ENTRY_ID != 0
                        {
                            ...
                            {
                                apply_smoothed_playable_masterhand_input(
                                    module_accessor,
                                    boss_boma,
                                    1.0,
                                );
