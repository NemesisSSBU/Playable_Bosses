...
/// Bounded WOL presentation motion probe for Dharkon. Evidence only: it reads
...
unsafe fn ensure_wol_dharkon_preview(
    ...
    if let Some((_, _, held_boma)) =
        boss_helpers::held_item_by_kind(module_accessor, &[*ITEM_KIND_DARZCENTIPEDE])
    {
