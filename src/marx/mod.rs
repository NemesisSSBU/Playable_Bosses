use crate::{config::CONFIG, MAX_PLAYERS, SELECTED_UI_CHARA};
...
pub static mut FIGHTER_NAME: [u64; MAX_PLAYERS] = [0; MAX_PLAYERS];
...
const CONTROL_SPEED_MUL: f32 = 2.0;
const CONTROL_SPEED_MUL_2: f32 = 0.05;

static MARX_NAME_HASHES: Lazy<[u64; 8]> = Lazy::new(|| {
    [
        hash40("MARX"),
        hash40("マルク"),
        hash40("MAX"),
...
#[inline(always)]
fn is_marx_name(hash: u64) -> bool {
    MARX_NAME_HASHES.iter().any(|candidate| *candidate == hash)
}

pub static TITLE_VERSION: Lazy<(u16, u16, u16)> = Lazy::new(|| unsafe {
    Initialize();
...
pub unsafe fn get_player_number(
    ...
    if smash::app::utility::get_kind(module_accessor) == *WEAPON_KIND_PTRAINER_PTRAINER {
        player_number = WorkModule::get_int(
            module_accessor,
        ...
        let mut owner_module_accessor = &mut *sv_battle_object::module_accessor(
        ...
        while get_category(owner_module_accessor) != *BATTLE_OBJECT_CATEGORY_FIGHTER {
            //Keep checking the owner of the boma we're working with until we've hit a boma that belongs to a fighter
            owner_module_accessor = &mut *sv_battle_object::module_accessor(
                (WorkModule::get_int(
...
extern "C" fn once_per_fighter_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            ENTRY_ID = WorkModule::get_int(module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID)
                as usize;
            LookupSymbol(
            ...
            let player_number = get_player_number(&mut *fighter.module_accessor);
            FIGHTER_NAME[player_number] =
                hash40(&read_tag(name_base + 0x260 * player_number as u64 + 0x8e));

            let selected_hash = SELECTED_UI_CHARA[ENTRY_ID];
            let is_marx_selection = selected_hash == hash40("ui_chara_marx");
            let fallback_name_match = is_marx_name(FIGHTER_NAME[player_number]);

            if !(is_marx_selection || (selected_hash == 0 && fallback_name_match)) {
                FIGHTER_NAME[player_number] = 0;
                EXISTS_PUBLIC = false;
                STOP = false;
                DEAD = false;
            ...
            if is_marx_selection || (selected_hash == 0 && fallback_name_match) {
                if smash::app::stage::get_stage_id() == 0x139 {
                        ...
                        || !ItemModule::is_have_item(module_accessor, 0)
                    {
                        ...
                        BOSS_ID[entry_id(module_accessor)] =
                            ItemModule::get_have_item_id(module_accessor, 0) as u32;
                        ModelModule::set_scale(module_accessor, 0.0001);
                        let boss_boma =
                            sv_battle_object::module_accessor(BOSS_ID[entry_id(module_accessor)]);
                            ...
                            == *FIGHTER_STATUS_KIND_REBIRTH
                    {
                            ...
                            || CONFIG.options.boss_respawn.unwrap_or(false)
                        {
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_MARX_STATUS_KIND_MOVE_TELEPORT,
                                true,
                            );
                            ...
                            if !CONTROLLABLE {
                                if MotionModule::motion_kind(boss_boma) == smash::hash40("wait")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_up_loop")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_up_end")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_down_start")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_down_loop")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_up_start")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_down_end")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_right_start")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_right_loop")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_right_end")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_left_end")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_left_loop")
                                    || MotionModule::motion_kind(boss_boma)
                                        == smash::hash40("move_left_start")
                                {
                                    CONTROLLABLE = true;
                                }
                            }
                    ...
                    if sv_information::is_ready_go() == true {
                        if CONTROLLABLE == true {
                            ...
                            ) == true
                            {
                                ...
                                StatusModule::change_status_request_from_script(
                                    boss_boma,
                                    *ITEM_MARX_STATUS_KIND_MOVE_STRAIGHT,
                                    true,
                                );
                            }
                        }
                            ...
                            if MotionModule::frame(boss_boma) > 210.0 {
                                ...
                                if STOP == false && !CONFIG.options.boss_respawn.unwrap_or(false) {
                                            ...
                                            != *FIGHTER_STATUS_KIND_DEAD
                                    {
                                        ...
                                        SoundModule::stop_se(
                                            module_accessor,
                                            Hash40::new("hp_battle_damage_reaction"),
                                            0,
                                        );
                                        SoundModule::stop_se(
                                            module_accessor,
                    ...
                    let fighter_manager =
                        *(FIGHTER_MANAGER as *mut *mut smash::app::FighterManager);
                    ...
                    ) == false
                    {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_PLANT_START
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 8.0
                            {
                                // CONTROLLABLE = true;
                            }
                        }
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_AVOID_TELEPORT
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 8.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_BLACK_HOLE_START
                        {
                            MotionModule::set_rate(boss_boma, 1.0);
                            smash::app::lua_bind::ItemMotionAnimcmdModuleImpl::set_fix_rate(
                                boss_boma, 1.0,
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_BLACK_HOLE_LOOP
                        {
                            CONTROLLABLE = false;
                        }
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_BLACK_HOLE_END
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 8.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_FACET_EYE_LASER_END
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 8.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_FLY_OUT
                        {
                            if MotionModule::frame(boss_boma)
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_FLY_OUT_HOMING
                        {
                            //Boss Control Stick Movement
                            if ControlModule::get_stick_x(module_accessor) <= 0.001 {
                                let pos = Vector3f {
                                    x: ControlModule::get_stick_x(module_accessor) * 1.75,
                                    y: 0.0,
                                    z: 0.0,
                                };
                                ...
                                && !DEAD
                            {
                                StatusModule::change_status_request_from_script(
                                    boss_boma,
                                    *ITEM_MARX_STATUS_KIND_ATTACK_FLY_OUT_HOMING_END,
                                    true,
                                );
                            }
                        }
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_CAPILLARY_END
                            && !DEAD
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_FOLLOW_EYE_END
                            && !DEAD
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_ICE_BOMB_SHOOT
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_4_CUTTER
                        {
                            MotionModule::set_rate(boss_boma, 1.0);
                            smash::app::lua_bind::ItemMotionAnimcmdModuleImpl::set_fix_rate(
                                boss_boma, 1.0,
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_ICE_BOMB_LOOP
                        {
                            ...
                            if ControlModule::get_stick_x(module_accessor) <= 0.001 {
                                let pos = Vector3f {
                                    x: ControlModule::get_stick_x(module_accessor) * 3.0,
                                    y: 0.0,
                                    z: 0.0,
                                };
                                PostureModule::add_pos(boss_boma, &pos);
                            ...
                            if ControlModule::get_stick_y(module_accessor) <= 0.001 {
                                let pos = Vector3f {
                                    x: 0.0,
                                    y: ControlModule::get_stick_y(module_accessor) * 3.0,
                                    z: 0.0,
                                };
                                PostureModule::add_pos(boss_boma, &pos);
                            }
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_MARX_STATUS_KIND_ATTACK_THICK_LASER_LOOP
                        {
                            //Boss Control Stick Movement
                            if ControlModule::get_stick_y(module_accessor) <= 0.001 {
                                let pos = Vector3f {
                                    x: 0.0,
                                    y: ControlModule::get_stick_y(module_accessor) * 1.0,
                                    z: 0.0,
                                };
                                PostureModule::add_pos(boss_boma, &pos);
                            }
                        ...
                        if CONTROLLABLE == true {
                            if DEAD == false {
                                ...
                                if lua_bind::PostureModule::lr(boss_boma) == -1.0 {
                                    // left
                                    let vec3 = Vector3f {
                                        x: CONTROLLER_Y / 2.0 * -15.0,
                                        y: 90.0 - CONTROLLER_X / 2.0 * 22.0,
                                        z: 0.0,
                                    };
                                    PostureModule::set_rot(boss_boma, &vec3, 0);
                                }
                                ...
                                if ControlModule::check_button_on(
                                    ...
                                    *CONTROL_PAD_BUTTON_ATTACK,
                                    ...
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_MARX_STATUS_KIND_ATTACK_ICE_BOMB_START,
                                        true,
                                    );
                                }
                                    ...
                                    != 0
                                {
                                    ...
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_MARX_STATUS_KIND_ATTACK_CAPILLARY_START,
                                        true,
                                    );
                                }
                                if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
                                    ...
                                    != 0
                                {
                                    ...
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_MARX_STATUS_KIND_ATTACK_THICK_LASER_START,
                                        true,
                                    );
                                }
                                if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
                                    ...
                                    != 0
                                {
                                    ...
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_MARX_STATUS_KIND_ATTACK_FACET_EYE_LASER_START,
                                        true,
                                    );
                                }
                                if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
                                    ...
                                    != 0
                                {
                                    ...
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_MARX_STATUS_KIND_ATTACK_BLACK_HOLE_READY,
                                        true,
                                    );
                                }
                                if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
                                    ...
                                    != 0
                                {
                                    ...
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_MARX_STATUS_KIND_ATTACK_FOLLOW_EYE_START,
                                        true,
                                    );
                                }
                                if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
