...
pub fn once_per_fighter_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            || FIGHTER_NAME[get_player_number(module_accessor)] == hash40("KILAIRE")
            || FIGHTER_NAME[get_player_number(module_accessor)] == hash40("KYRA")
            || FIGHTER_NAME[get_player_number(module_accessor)] == hash40("LÚMINA")
            || FIGHTER_NAME[get_player_number(module_accessor)] == hash40("KIARAN")
            || FIGHTER_NAME[get_player_number(module_accessor)] == hash40("吉拉")
            || FIGHTER_NAME[get_player_number(module_accessor)] == hash40("키라")
            || FIGHTER_NAME[get_player_number(module_accessor)] == hash40("СИЯН") {
                if smash::app::stage::get_stage_id() == 0x139 {
                    ...
                    if sv_information::is_ready_go() == false {
                        ...
                        if MotionModule::frame(module_accessor) >= 5.0 && smash::app::smashball::is_training_mode() == false {
                            if ModelModule::scale(module_accessor) != 0.0001 {
                                ...
                                ModelModule::set_scale(boss_boma_2, 0.0001);
                                StatusModule::change_status_request_from_script(boss_boma_2, *ITEM_MASTERHAND_STATUS_KIND_WAIT_FEINT, true);
                    ...
                    if sv_information::is_ready_go() == true {
                        ...
                        if sv_information::is_ready_go() == true {
                            if DamageModule::damage(module_accessor, 0) >= 100.0 && !DEAD {
                                if STUNNED == false {
                                    CONTROLLABLE = false;
                                    STUNNED = true;
                                    StatusModule::change_status_request_from_script(boss_boma,*ITEM_KIILA_STATUS_KIND_DOWN_START,true);
                                }
                            }
                            ...
                            if StopModule::is_damage(boss_boma) | StopModule::is_damage(module_accessor) {
                                ...
                                WorkModule::set_flag(boss_boma, true, *ITEM_STATUS_WORK_FLAG_HIT);
                        ...
                        if sv_information::is_ready_go() == true && smash::app::smashball::is_training_mode() == false && !DEAD {
                            let x = PostureModule::pos_x(boss_boma);
                            let y = PostureModule::pos_y(boss_boma);
                            let z = PostureModule::pos_z(boss_boma);
                            let boss_pos = Vector3f{x: x, y: y, z: z};
                            let boss_boma_2 = sv_battle_object::module_accessor(BOSS_ID_2[entry_id(module_accessor)]);
                            PostureModule::set_pos(boss_boma_2, &boss_pos);
                            StatusModule::change_status_request_from_script(boss_boma_2, *ITEM_MASTERHAND_STATUS_KIND_WAIT_FEINT, true);
                        }
                        ...
                        if CONTROLLABLE == true && FighterInformation::is_operation_cpu(FighterManager::get_fighter_information(fighter_manager,smash::app::FighterEntryID(ENTRY_ID as i32))) == false {
                            if DEAD == false {
                                //Boss Control Stick Movement
                                if ControlModule::get_stick_x(module_accessor) <= 0.001 {
