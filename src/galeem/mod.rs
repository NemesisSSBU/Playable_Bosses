...
#[inline(always)]
unsafe fn log_galeem_entry_phase(
    tag: &str,
    module_accessor: *mut BattleObjectModuleAccessor,
    boss_active: bool,
    ...
    crate::boss_log!(
        "[PB][Galeem][Phase] tag={} entry={} stage=0x{:x} ready_go={} fighter_status={} frame={:.2} scale={:.4} tracked_id=0x{:x} tracked_active={} tracked_status={} hidden_cpu=0x{:x} hidden_cpu_active={} hidden_cpu_status={} stage1={} stage2={} preparation_attempted={} staged_boss_prepared={} exists_public={} controllable={} dead={} stop={} result_spawned={}",
        tag,
        entry,
...
unsafe fn log_galeem_spawn_bisect(
    ...
    crate::boss_log!(
        "[PB][GaleemSpawnBisect] step={:02} edge={} name={} entry={} host_status={} tracked_id=0x{:x} hidden_cpu=0x{:x} boss_boma_present={}",
        step,
        edge,
        name,
        entry,
...
#[inline(always)]
unsafe fn log_galeem_spawn_state(
    tag: &str,
    module_accessor: *mut BattleObjectModuleAccessor,
    boss_boma: *mut BattleObjectModuleAccessor,
) {
...
unsafe fn prepare_staged_galeem_before_ready_go(
    ...
    if hidden_cpu_id == 0 || !sv_battle_object::is_active(hidden_cpu_id) {
        crate::boss_log!(
            "[PB][Galeem][StartupGate] action=prepare_failed entry={} reason=hidden_cpu_inactive hidden_cpu=0x{:x}",
            entry,
            hidden_cpu_id
        );
        return false;
    ...
    crate::boss_log!(
        "[PB][Galeem][StartupGate] action=prepared_inert entry={} ready_go=false hidden_cpu=0x{:x} boss_id=0x{:x} boss_kind={} boss_status={} motion=wait",
        entry,
        hidden_cpu_id,
        boss_id,
        acquired_kind,
...
unsafe fn activate_staged_galeem_after_ready_go(
    ...
    if hidden_cpu_id == 0 || !sv_battle_object::is_active(hidden_cpu_id) {
        crate::boss_log!(
            "[PB][Galeem][StartupGate] action=activate_failed entry={} reason=hidden_cpu_inactive hidden_cpu=0x{:x}",
            entry,
            hidden_cpu_id
        );
        return false;
    ...
    crate::boss_log!(
        "[PB][Galeem][StartupGate] action=activated entry={} ready_go=true hidden_cpu=0x{:x} boss_id=0x{:x} boss_status={}",
        entry,
        hidden_cpu_id,
        boss_id,
        StatusModule::status_kind(boss_boma)
...
#[inline(always)]
unsafe fn teardown_galeem_post_match_transition(
    module_accessor: *mut BattleObjectModuleAccessor,
) -> bool {
    if module_accessor.is_null() {
        return false;
                ...
                if boss_helpers::is_boss_preview_stage(stage_id) {
                            ...
                            == *FIGHTER_STATUS_KIND_REBIRTH
                    {
                            ...
                            || CONFIG.options.boss_respawn.unwrap_or(false)
                        {
                            ...
                            log_galeem_spawn_state("rebirth", module_accessor, boss_boma);
                    ...
                    if DEAD == false {
                            ...
                            && BOSS_ID[boss_helpers::entry_id(module_accessor)] != 0
                        {
                            ...
                            if !boss_boma.is_null()
                                && StatusModule::status_kind(boss_boma)
                                    == *ITEM_KIILA_STATUS_KIND_DOWN_LOOP
                            {
                        ...
                        && BOSS_ID[boss_helpers::entry_id(module_accessor)] != 0
                    {
                        ...
                        if !boss_boma.is_null() {
                            ...
                            if StatusModule::status_kind(boss_boma)
                                == *ITEM_KIILA_STATUS_KIND_DOWN_START
                            {
                                CONTROLLABLE = false;
                            }
                            if StatusModule::status_kind(boss_boma)
                            ...
                            if StatusModule::status_kind(boss_boma) == 63
                                && StatusModule::status_kind(boss_boma)
                                    != *ITEM_KIILA_STATUS_KIND_CRUSH_DOWN
                                && !CONTROLLABLE
                            {
                                StatusModule::change_status_request_from_script(
                                    boss_boma,
                                ...
                                == true
                            {
                                if DEAD == false {
                                    if CONTROLLABLE == true {
                                        ...
                                        {
                                            RANDOM_ATTACK =
                                                smash::app::sv_math::rand(hash40("fighter"), 11);
                                            if RANDOM_ATTACK == 0 {
                                                CONTROLLABLE = false;
                                                StatusModule::change_status_request_from_script(
                                                    boss_boma,
                                            ...
                                            if RANDOM_ATTACK == 10 {
                                                ...
                                                StatusModule::change_status_request_from_script(
                                                    boss_boma,
                                                    *ITEM_KIILA_STATUS_KIND_CRUSH_DOWN_START,
                                                    true,
                                                );
                                            }
                                        }
