...
#[inline(always)]
unsafe fn restore_galeem_after_item_wipe(
    module_accessor: *mut BattleObjectModuleAccessor,
) {
...
#[inline(always)]
unsafe fn cleanup_galeem_result_state(
    module_accessor: *mut BattleObjectModuleAccessor,
) {
    if module_accessor.is_null() {
        return;
                ...
                if boss_helpers::is_boss_preview_stage(smash::app::stage::get_stage_id()) {
                    ...
                    if sv_information::is_ready_go() == false {
                        ...
                        if smash::app::smashball::is_training_mode() == false {
                                ...
                                && stage_one_prepared
                            {
                                ...
                                log_galeem_entry_phase("stage2_prepare", module_accessor, false, false, true);
                            }
                            if MotionModule::frame(module_accessor) >= 5.0
                                && !boss_active
                                && boss_helpers::is_hidden_host_entry_stage_two(module_accessor)
                            {
                                log_galeem_entry_phase("initial_spawn_start", module_accessor, false, false, true);
                                DEAD = false;
                                CONTROLLABLE = true;
                                JUMP_START = false;
                                STOP = false;
                    ...
                    || CONFIG.options.boss_respawn.unwrap_or(false)) {
                        ...
                        if !boss_active && !stage_one_prepared && !stage_two_prepared {
                            ...
                            ENTRY_ID = WorkModule::get_int(module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID) as usize;
                            ModelModule::set_scale(module_accessor, boss_helpers::HIDDEN_HOST_ENTRY_PREP_SCALE);
                            log_galeem_entry_phase("ready_go_stage1", module_accessor, false, true, false);
                        }
                        if !boss_active && stage_one_prepared {
                            ...
                            log_galeem_entry_phase("ready_go_stage2", module_accessor, false, false, true);
                        }
                        if !boss_active && stage_two_prepared {
                            log_galeem_entry_phase("ready_go_spawn_start", module_accessor, false, false, true);
                            RESULT_SPAWNED = false;
                            let boss_boma = boss_helpers::acquire_boss_item(
                                module_accessor,
                                &raw mut BOSS_ID,
