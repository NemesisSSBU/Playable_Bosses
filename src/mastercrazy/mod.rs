...
use crate::{MAX_PLAYERS, SELECTED_UI_CHARA};
...
static mut BOSS_ID_2: [u32; MAX_PLAYERS] = [0; MAX_PLAYERS];
pub static mut FIGHTER_NAME_2: [u64; MAX_PLAYERS] = [0; MAX_PLAYERS];
...
extern "C" fn once_per_fighter_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            FIGHTER_NAME[get_player_number(&mut *fighter.module_accessor)] = hash40(&read_tag(
                name_base + 0x260 * get_player_number(&mut *fighter.module_accessor) as u64 + 0x8e,
            ));
            let selected_hash = SELECTED_UI_CHARA[ENTRY_ID];
            let is_master_selection = selected_hash == hash40("ui_chara_masterhand");
...
extern "C" fn once_per_fighter_frame_2(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            let name_base = get_version_offset();
            FIGHTER_NAME_2[get_player_number(&mut *fighter.module_accessor)] = hash40(&read_tag(
                name_base + 0x260 * get_player_number(&mut *fighter.module_accessor) as u64 + 0x8e,
            ));
            let selected_hash = SELECTED_UI_CHARA[ENTRY_ID_2];
            let is_crazy_selection = selected_hash == hash40("ui_chara_crazyhand");
            if is_crazy_selection {
                if smash::app::stage::get_stage_id() == 0x139 {
                    let lua_state = fighter.lua_state_agent;
                    let module_accessor =
                        smash::app::sv_system::battle_object_module_accessor(lua_state);
                        ...
                        || !ItemModule::is_have_item(module_accessor, 0)
                    {
                        ...
                        BOSS_ID_2[entry_id(module_accessor)] =
                            ItemModule::get_have_item_id(module_accessor, 0) as u32;
                        ModelModule::set_scale(module_accessor, 0.0001);
                        let boss_boma_2 =
                            sv_battle_object::module_accessor(BOSS_ID_2[entry_id(module_accessor)]);
                            ...
                            == *FIGHTER_STATUS_KIND_REBIRTH
                    {
                            ...
                            || CONFIG.options.boss_respawn.unwrap_or(false)
                        {
                            ...
                            if FighterInformation::is_operation_cpu(
                                FighterManager::get_fighter_information(
                                    fighter_manager,
                                    smash::app::FighterEntryID(ENTRY_ID_2 as i32),
                                ),
                            ) == false
                            {
                                CONTROLLABLE_2 = true;
                    ...
                    if sv_information::is_ready_go() == true {
                        ...
                        if MotionModule::is_end(boss_boma_2)
                            && MotionModule::motion_kind(boss_boma_2) == hash40("electroshock_end")
                            && !DEAD_2
                        {
                            if FighterInformation::is_operation_cpu(
                                FighterManager::get_fighter_information(
                    ...
                    if sv_information::is_ready_go() == true && !DEAD_2 {
                            ...
                            == *ITEM_CRAZYHAND_STATUS_KIND_YUBI_BEAM
                        {
                            ...
                            if CONTROLLER_X_CRAZY
                                ...
                                && ControlModule::get_stick_x(module_accessor) > 0.0
                            {
                                CONTROLLER_X_CRAZY += (ControlModule::get_stick_x(module_accessor)
                                    * CONTROL_SPEED_MUL)
                                    * CONTROL_SPEED_MUL_2;
                            }
                            if CONTROLLER_X_CRAZY
                            ...
                            if CONTROLLER_Y_CRAZY
                                ...
                                && ControlModule::get_stick_y(module_accessor) > 0.0
                            {
                                CONTROLLER_Y_CRAZY += (ControlModule::get_stick_y(module_accessor)
                                    * CONTROL_SPEED_MUL)
                                    * CONTROL_SPEED_MUL_2;
                            }
                            if CONTROLLER_Y_CRAZY
