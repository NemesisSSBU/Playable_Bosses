...
static DRACULA_NAME_HASHES: Lazy<[u64; 4]> = Lazy::new(|| {
    [
        hash40("DRACULA"),
        hash40("ドラキュラ"),
        hash40("DRÁCULA"),
...
#[inline(always)]
fn is_dracula_name(hash: u64) -> bool {
    DRACULA_NAME_HASHES
        .iter()
...
extern "C" fn once_per_fighter_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            let is_dracula_selection = selected_hash == hash40("ui_chara_dracula");
            let fallback_name_match =
                selected_hash == 0 && is_dracula_name(FIGHTER_NAME[player_number]);

            if !(is_dracula_selection || fallback_name_match) {
            ...
            if smash::app::stage::get_stage_id() == 0x139 {
                ...
                if ModelModule::scale(module_accessor) == 0.0001 {
                    ...
                    let module_pos = Vector3f {
                        ...
                        y: PostureModule::pos_y(module_accessor) - 8.5,
                ...
                if sv_information::is_ready_go() == true {
                    if !TRANSFORMED_MODE {
                        ...
                        if DamageModule::damage(module_accessor, 0) >= 120.0 {
                            if !DEAD
                                && !TRANSFORMED_MODE
                                && !WorkModule::is_flag(boss_boma, *ITEM_INSTANCE_WORK_FLAG_ANGRY)
                            {
                ...
                if StatusModule::status_kind(boss_boma) == *ITEM_DRACULA2_STATUS_KIND_SQUASH_START
                    && TRANSFORMED_MODE
                {
                    ...
                    && DEAD == false
                {
                    ...
                    if StatusModule::status_kind(boss_boma) == *ITEM_DRACULA2_STATUS_KIND_STEP_SLASH
                        && TRANSFORMED_MODE
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
