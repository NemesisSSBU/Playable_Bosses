...
extern "C" fn once_per_fighter_frame(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            if selected_hash != hash40("ui_chara_lioleus") {
                CONTROLLABLE = true;
                DEAD = false;
                EXISTS_PUBLIC = false;
                BOSS_ID[ENTRY_ID] = 0;
            ...
            if smash::app::stage::get_stage_id() == 0x139 {
                    ...
                    || !ItemModule::is_have_item(module_accessor, 0)
                {
                    ...
                    ItemModule::have_item(
                        module_accessor,
                        ItemKind(*ITEM_KIND_LIOLEUSBOSS),
                        0,
                        0,
                        false,
                        false,
                    ...
                    MotionModule::change_motion(
                        boss_boma,
                        smash::phx::Hash40::new("hovering_move"),
                        0.0,
                        1.0,
                        false,
                        0.0,
                        ...
                        if CONTROLLABLE == true {
                            if GROUNDED == true {
                                ...
                                if ControlModule::get_stick_x(module_accessor) > -0.1
                                    && ControlModule::get_stick_x(module_accessor) < 0.1
                                    && MotionModule::motion_kind(boss_boma)
                                        != smash::hash40("hovering")
                                {
                                    MotionModule::set_rate(boss_boma, 1.0);
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_LIOLEUSBOSS_STATUS_KIND_DEBUG_CONTROL,
                                        true,
                                    );
                                    MotionModule::change_motion(
                                        boss_boma,
                ...
                if DEAD == false {
                    if sv_information::is_ready_go() == true && !DEAD {
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_LOOP
                        {
                            let stunned = !CONFIG.options.full_stun_duration.unwrap_or(false);
                            if stunned {
                                StatusModule::change_status_request_from_script(
                                    boss_boma,
                                    *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_END,
                                    true,
                                );
                            }
                            CONTROLLABLE = false;
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_AIR_LOOP
                        {
                            let stunned = !CONFIG.options.full_stun_duration.unwrap_or(false);
                            if stunned {
                                StatusModule::change_status_request_from_script(
                                    boss_boma,
                                    *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_AIR_END,
                                    true,
                                );
                            }
                            CONTROLLABLE = false;
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_TAIL_CUT_START
                        {
                            if MotionModule::frame(boss_boma) >= MotionModule::end_frame(boss_boma)
                            {
                                ...
                                if stunned {
                                    StatusModule::change_status_request_from_script(
                                        boss_boma,
                                        *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_TAIL_CUT_END,
                                        true,
                                    );
                                }
                                CONTROLLABLE = false;
                ...
                if sv_information::is_ready_go() == true && !DEAD {
                    if CONTROLLABLE == true {
                        ...
                        ) == true
                        {
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_HOWLING,
                                true,
                            );
                        }
                    }
                ...
                if FighterInformation::is_operation_cpu(FighterManager::get_fighter_information(
                    fighter_manager,
                    smash::app::FighterEntryID(ENTRY_ID as i32),
                )) == false
                    && DEAD == false
                    ...
                    && DEAD == false
                {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_START
                    {
                        CONTROLLABLE = false;
                    }
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_MOVE_TACKLE_TURN
                    {
                        GROUNDED = true;
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_HOLE_END
                    {
                        GROUNDED = true;
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_MOVE_TACKLE_END
                    {
                        GROUNDED = true;
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_GLIDE_END
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_FIREBALL_AIR
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_GLIDE_END2
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_TACKLE_END
                    {
                        GROUNDED = true;
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_FIREBALL3_AIR_END
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_CHARGE_FIREBALL_END
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_GLIDE_START
                    {
                        //Boss Control Stick Movement
                        if ControlModule::get_stick_x(module_accessor) <= 0.001 {
                            let pos = Vector3f {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_HOWLING_AIR
                    {
                        //Boss Control Stick Movement
                        if ControlModule::get_stick_x(module_accessor) <= 0.001 {
                            let pos = Vector3f {
                                x: ControlModule::get_stick_x(module_accessor) * 0.2,
                                y: 0.0,
                                z: 0.0,
                            };
                            PostureModule::add_pos(boss_boma, &pos);
                        ...
                        if ControlModule::get_stick_y(module_accessor) <= 0.001 {
                            let pos = Vector3f {
                                x: 0.0,
                                y: ControlModule::get_stick_y(module_accessor) * 0.2,
                                z: 0.0,
                            };
                            PostureModule::add_pos(boss_boma, &pos);
                        }
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_NAIL_AIR
                    {
                        //Boss Control Stick Movement
                        if ControlModule::get_stick_x(module_accessor) <= 0.001 {
                            let pos = Vector3f {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_TURN_AIR
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_AIR_START
                    {
                        CONTROLLABLE = false;
                    }
                    if StatusModule::status_kind(boss_boma)
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_BACK_JUMP_FIREBALL
                    {
                        GROUNDED = true;
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_BACK_JUMP_FIREBALL2
                    {
                        GROUNDED = true;
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_TACKLE_JUMP
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_DEBUG_ATTACK_TACKLE_JUMP
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_CHANGE_MODE_AIR
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if StatusModule::status_kind(boss_boma)
                        == *ITEM_LIOLEUSBOSS_STATUS_KIND_CHANGE_MODE_GROUND
                    {
                        if MotionModule::frame(boss_boma)
                            >= MotionModule::end_frame(boss_boma) - 2.0
                        {
                    ...
                    if CONTROLLABLE == false {
                        if GROUNDED == true {
                            if DEAD == false {
                                if StatusModule::status_kind(boss_boma)
                                    == *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_TACKLE
                                {
                                    if lua_bind::PostureModule::lr(boss_boma) == 1.0 {
                                        ...
                                        if ControlModule::get_stick_x(module_accessor) < -0.1 {
                                            StatusModule::change_status_request_from_script(
                                                boss_boma,
                                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_TACKLE_TURN,
                                                true,
                                            );
                                        }
                                    }
                        ...
                        && sv_information::is_ready_go() == true
                    {
                        ...
                        if ControlModule::check_button_on(
                            ...
                            *CONTROL_PAD_BUTTON_GUARD,
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_TURN,
                                true,
                            );
                        }
                        if ControlModule::check_button_on(
                            ...
                            *CONTROL_PAD_BUTTON_ATTACK,
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_TACKLE_START,
                                true,
                            );
                        }
                            ...
                            != 0
                        {
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_ASSAULT_START,
                                true,
                            );
                        }
                        if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
                            ...
                            != 0
                        {
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_FIREBALL,
                                true,
                            );
                        }
                        if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
                            ...
                            != 0
                        {
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_FIREBALL3_START,
                                true,
                            );
                        }
                        if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
                        ...
                        if ControlModule::check_button_on(
                            ...
                            *CONTROL_PAD_BUTTON_APPEAL_HI,
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_CHARGE_FIREBALL_START,
                                true,
                            );
                        }
                    }
                        ...
                        && sv_information::is_ready_go() == true
                    {
                        ...
                        if ControlModule::check_button_on(
                            ...
                            *CONTROL_PAD_BUTTON_SPECIAL,
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_CHARGE_FIREBALL_START_AIR,
                                true,
                            );
                        }
                        if ControlModule::check_button_on(
                            ...
                            != 0
                        {
                            ...
                            StatusModule::change_status_request_from_script(
                                boss_boma,
                                *ITEM_LIOLEUSBOSS_STATUS_KIND_ATTACK_FIREBALL3_AIR_START,
                                true,
                            );
                        }
                        if ControlModule::get_command_flag_cat(fighter.module_accessor, 0)
