...
unsafe fn restore_rathalos_after_item_wipe(module_accessor: *mut BattleObjectModuleAccessor) {
    ...
    STOP = false;
    let boss_boma =
        boss_helpers::acquire_boss_item(module_accessor, &raw mut BOSS_ID, *ITEM_KIND_LIOLEUSBOSS);
            ...
            if selected_via_slot {
                ...
                } else if !boss_helpers::is_boss_passthrough_stage(smash::app::stage::get_stage_id())
                {
                    ...
                    if DEAD == false {
                        if sv_information::is_ready_go() == true && !DEAD {
                                ...
                                == *ITEM_LIOLEUSBOSS_STATUS_KIND_DOWN_TAIL_CUT_START
                            {
                                ...
                                {
                                    let stunned =
                                        !CONFIG.options.full_stun_duration.unwrap_or(false);
                                    if stunned {
                                        StatusModule::change_status_request_from_script(
                                            boss_boma,
                        ...
                        && DEAD == false
                    {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_LIOLEUSBOSS_STATUS_KIND_FLY_END
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 2.0
                            {
