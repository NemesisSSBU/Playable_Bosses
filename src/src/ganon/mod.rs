...
pub unsafe fn check_status() -> bool {
    EXISTS_PUBLIC || boss_runtime::any_exists_public(&raw const boss_runtime::GANON_RUNTIME)
}

#[inline(always)]
unsafe fn load_ganon_runtime(slot: *mut BossCommonRuntime) {
    if slot.is_null() {
        return;
    }
...
#[inline(always)]
unsafe fn store_ganon_runtime(slot: *mut BossCommonRuntime) {
    if slot.is_null() {
        return;
    }
...
#[inline(always)]
unsafe fn restore_ganon_after_item_wipe(module_accessor: *mut BattleObjectModuleAccessor) {
    ...
    if let Some((_, held_id, _)) =
        boss_helpers::held_item_by_kind(module_accessor, &[*ITEM_KIND_GANONBOSS])
    {
    ...
    RETURN = false;
    let boss_boma =
        boss_helpers::acquire_boss_item(module_accessor, &raw mut BOSS_ID, *ITEM_KIND_GANONBOSS);
    ...
    crate::boss_log!(
        "[PB][Recover] entry {}: restored Ganon after item wipe",
        entry
    );
}
    ...
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            let _runtime_guard = CommonRuntimeSyncGuard::new(
                boss_runtime::slot_ptr(&raw mut boss_runtime::GANON_RUNTIME, ENTRY_ID),
                load_ganon_runtime,
                store_ganon_runtime,
            );
            let fighter_manager = boss_helpers::fighter_manager();
            ...
            if selected_via_slot {
                ...
                if boss_helpers::is_boss_preview_stage(stage_id) {
                    ...
                    if sv_information::is_ready_go() == true && !DEAD {
                        if boss_helpers::is_operation_cpu_entry(fighter_manager, ENTRY_ID) == false
                        {
                            ...
                            if StatusModule::status_kind(boss_boma)
                                != *ITEM_GANONBOSS_STATUS_KIND_DOWN_START
                                && StatusModule::status_kind(boss_boma)
                                    != *ITEM_GANONBOSS_STATUS_KIND_DOWN_LOOP
                                && StatusModule::status_kind(boss_boma)
                                    != *ITEM_GANONBOSS_STATUS_KIND_DOWN_END
                            {
                                if CONTROLLABLE {
                                    ...
                                    {
                                        if StatusModule::status_kind(boss_boma)
                                            != *ITEM_GANONBOSS_STATUS_KIND_WALK_FRONT
                                            || StatusModule::status_kind(boss_boma)
                                                != *ITEM_GANONBOSS_STATUS_KIND_WALK_BACK
                                        {
                                            if MOVING == false {
                                                if FRESH_CONTROL {
                                                    StatusModule::change_status_request_from_script(
                    ...
                    if sv_information::is_ready_go() == true && !DEAD {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_BACK_JUMP
                        {
                            if MotionModule::frame(boss_boma)
                    ...
                    if boss_helpers::is_operation_cpu_entry(fighter_manager, ENTRY_ID) == false {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_DOUBLE_SLASH_EXEC
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_DOUBLE_SLASH_START
                        {
                            //Boss Control Stick Movement
                            if ControlModule::get_stick_x(module_accessor) <= 0.001 {
                                let pos = Vector3f {
                                    x: ControlModule::get_stick_x(module_accessor) * 1.0,
                                    y: 0.0,
                                    z: 0.0,
                                };
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_SPIN_SLASH_LOOP
                        {
                            //Boss Control Stick Movement
                            if ControlModule::get_stick_x(module_accessor) <= 0.001 {
                                let pos = Vector3f {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_SPIN_SLASH_END_REVERSE
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_BODY_ATTACK_END
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_TURN_JUMP
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_RETURN_JUMP
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_SPIN_SLASH_END
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_LASER_BEAM_END
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_BIG_JUMP_LANDING
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_HOMING_BOMB_BACK
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_HOMING_BOMB_TURN
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_THUNDER_SLASH_RETURN
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_THUNDER_SLASH_EXEC
                        {
                            if RETURN
                                && MotionModule::frame(boss_boma)
                                    >= MotionModule::end_frame(boss_boma) - 85.0
                            {
                                RETURN = false;
                                StatusModule::change_status_request_from_script(
                                    boss_boma,
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_JUMP_SLASH_EXEC
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_BACK_SLASH
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GANONBOSS_STATUS_KIND_ATTACK_SLASH_UP
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                                ...
                                if DEAD == false {
                                    ...
                                    if ControlModule::check_button_on(
                                        ...
                                        *CONTROL_PAD_BUTTON_SPECIAL,
                                        ...
                                        StatusModule::change_status_request_from_script(
                                            boss_boma,
                                            *ITEM_GANONBOSS_STATUS_KIND_ATTACK_JUMP_SLASH_START,
                                            true,
                                        );
                                    }
                                    if ControlModule::check_button_on(
                                    ...
                                    if ControlModule::check_button_on(
                                        ...
                                        *CONTROL_PAD_BUTTON_ATTACK,
                                        ...
                                        StatusModule::change_status_request_from_script(
                                            boss_boma,
                                            *ITEM_GANONBOSS_STATUS_KIND_ATTACK_LASER_BEAM_HOLD,
                                            true,
                                        );
                                    }
                                        ...
                                        != 0
                                    {
                                        ...
                                        StatusModule::change_status_request_from_script(
                                            boss_boma,
                                            *ITEM_GANONBOSS_STATUS_KIND_ATTACK_SPIN_SLASH_START,
                                            true,
                                        );
                                    }
                                    if ControlModule::get_command_flag_cat(
                                        ...
                                        != 0
                                    {
                                        ...
                                        StatusModule::change_status_request_from_script(
                                            boss_boma,
                                            *ITEM_GANONBOSS_STATUS_KIND_ATTACK_HOMING_BOMB_HOLD,
                                            true,
                                        );
                                    }
                                    if ControlModule::get_command_flag_cat(
                                        ...
                                        != 0
                                    {
                                        ...
                                        StatusModule::change_status_request_from_script(
                                            boss_boma,
                                            *ITEM_GANONBOSS_STATUS_KIND_ATTACK_THUNDER_SLASH_START,
                                            true,
                                        );
                                    }
                                    if ControlModule::get_command_flag_cat(
                                    ...
                                    if ControlModule::check_button_on(
                                        ...
                                        *CONTROL_PAD_BUTTON_APPEAL_S_L,
                                        ...
                                        StatusModule::change_status_request_from_script(
                                            boss_boma,
                                            *ITEM_GANONBOSS_STATUS_KIND_ATTACK_THUNDER_SLASH_TURN,
                                            true,
                                        );
                                    }
                                    if ControlModule::check_button_on(
