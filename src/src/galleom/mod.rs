...
pub unsafe fn check_status() -> bool {
    EXISTS_PUBLIC || boss_runtime::any_exists_public(&raw const boss_runtime::GALLEOM_RUNTIME)
}

#[inline(always)]
unsafe fn load_galleom_runtime(slot: *mut BossCommonRuntime) {
    if slot.is_null() {
        return;
    }
...
#[inline(always)]
unsafe fn store_galleom_runtime(slot: *mut BossCommonRuntime) {
    if slot.is_null() {
        return;
    }
...
unsafe fn restore_galleom_after_item_wipe(module_accessor: *mut BattleObjectModuleAccessor) {
    ...
    if let Some((_, held_id, _)) =
        boss_helpers::held_item_by_kind(module_accessor, &[*ITEM_KIND_GALLEOM])
    {
    ...
    STOP = false;
    let boss_boma =
        boss_helpers::acquire_boss_item(module_accessor, &raw mut BOSS_ID, *ITEM_KIND_GALLEOM);
    ...
    crate::boss_log!(
        "[PB][Recover] entry {}: restored Galleom after item wipe",
        entry
    );
}
    ...
    unsafe {
        ...
        if fighter_kind == *FIGHTER_KIND_MARIO {
            ...
            let _runtime_guard = CommonRuntimeSyncGuard::new(
                boss_runtime::slot_ptr(&raw mut boss_runtime::GALLEOM_RUNTIME, ENTRY_ID),
                load_galleom_runtime,
                store_galleom_runtime,
            );
            let fighter_manager = boss_helpers::fighter_manager();
            ...
            if selected_via_slot {
                ...
                if boss_helpers::is_boss_preview_stage(stage_id) {
                    ...
                    if boss_helpers::is_operation_cpu_entry(fighter_manager, ENTRY_ID) == false {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GALLEOM_STATUS_KIND_SHOOT_END
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GALLEOM_STATUS_KIND_GRAB_MISS
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
                        ...
                        if StatusModule::status_kind(boss_boma)
                            == *ITEM_GALLEOM_STATUS_KIND_GRAB_CRUSH
                        {
                            if MotionModule::frame(boss_boma)
                                >= MotionModule::end_frame(boss_boma) - 5.0
                            {
